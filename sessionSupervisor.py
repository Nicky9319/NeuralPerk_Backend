...
class sessionSupervisor():
    ...
    def broadcast(self , message):
        for users in self.userList:
            response = requests.post(f"http://127.0.0.1:7777/sendMessageToParticularUser" , json = message)
            if response == 200:
    ...
    def handle_model_training(self , modelData):
        setupMessage = {"TYPE" : "MODEL_SETUP" , "DATA" : modelData , "TOTAL_USERS" : len(self.userList) , "TIME" : time.time() , "CUSTOMER" : self.customerEmail}
        self.broadcast(pickle.dumps(setupMessage))

        trainingMessage = {"TYPE" : "TRAIN" , "EPOCHS" : self.epochs , "DATA" : self.model.get_weights() , "TIME" : time.time()}
        self.broadcast(pickle.dumps(trainingMessage))
    ...
    def runAndManageSession(self , data):
        ...
        response = requests.put("http://127.0.0.1:6666/updateSessionStatus" , json = {"EMAIL" : self.customerEmail , "STATUS" : "IDLE"})
